...
export class VocabularyListsComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    ...
    this.weekSelectorStyle = _.map(this._dataMgr.weekData.allWeek, week => {
      let tmp: object;

      if (this._dataMgr.getWeekType(week) == 'noPoly')
        tmp = {title: `Week ${week}`, cls: 'disabled', disabled: true};
      else if (week <= this._dataMgr.weekData.thisWeek)
        tmp = {title: `Week ${week}`, cls: 'default'};
      else
        tmp = {title: `Week ${week}`, cls: 'disabled', disabled: true};

      if (week == this._dataMgr.weekData.thisWeek) {
        tmp = {title: `Week ${week}`, cls: 'this-week'};
  ...
  private isTeachersPrintCall() {
    const URLParam = new URLSearchParams(window.location.search);
    return (URLParam.get('teacherYN') == 'Y' && URLParam.get('comp') == 'vocablists-print'
      && (URLParam.get('activityCode') == 'PF1' || URLParam.get('activityCode') == 'RF1' || URLParam.get('activityCode') == 'MF2'));
  }
  ...
  public setCardViewMode(idx: number) {
    ...
    //console.log("this._dataMgr: ", this._dataMgr)
    //console.log("파일명: ", this.vocabData['vocabs'][this.viewCardIdx]['SOUND_PATH'])
